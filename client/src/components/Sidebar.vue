...
const menus = [
  { label: "Documents", url: "/list", icon: ["far", "file"] },
  { label: "Settings", url: "/settings", icon: ["fas", "gear"] },
]
  ...
  <nav
    ...
    <div class="divide-y">
      ...
      <div class="flex flex-col gap-1 py-2">
        <div v-for="menu in menus" :key="menu.url" class="flex items-center mx-2 gap-3 cursor-pointer rounded-lg border"
          :class="activeMenu.startsWith(menu.url) ? 'bg-indigo-50 text-indigo-500 shadow-sm border-indigo-100' : 'text-slate-500 hover:bg-gray-100 border-transparent'"
    ...
    <div class="flex items-center p-2 hover:bg-gray-100 border-t cursor-pointer" @click="toggleSidebar">
      <div class="size-10 flex items-center justify-center rounded-lg">
        <font-awesome-icon class="text-slate-500" :class="isSidebarExpanded ? 'rotate-180' : ''"
