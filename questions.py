...
async def extract_section_data(model, user_pdf_content: bytes, section_info: dict) -> dict:
    # Get base64 of user input PDF
    pdf_content_base64 = base64.standard_b64encode(user_pdf_content).decode("utf-8")
    ...
    section_prompt = [
        ...
        {"text": "Below is how the above reference PDF section was correctly extracted:"},
        {"text": reference_structure},
        {"text": f"Now, ANALYZE YOUR TARGET PDF SECTION: {section_info['name']}, Pages {section_info['start_page']} to {section_info['end_page']}, and extract questions {question_range}:"},
        ...
        # """},
        {"text": "Return only a valid JSON object. No explanations. No markdown formatting. No unnecessary backslashes \\. Just JSON."}
    ]
